...
function handleCreepBuild() {
  if (roleUniversal.handleAutoBuild())
    return;
  if (roleMinerLevel2.handleAutoBuild())
    return;
  if (roleMinerLevel1.handleAutoBuild())
    return;
  if (roleSupplier.handleAutoBuild())
    return;
  if (roleCarryLevel1.handleAutoBuild())
    return;
  if (roleCarryLevel2.handleAutoBuild())
    return;
  if (roleBuilder.handleAutoBuild())
    return;
  if (roleUpgraderLevel2.handleAutoBuild())
    return;
  if (roleUpgraderLevel1.handleAutoBuild())
    return;
  if (roleDefenderRanged.handleAutoBuild())
    return;
}
...
export function handleRoadBuilding(room: Room) {
  if (!(getMainController().level == 3
    && Memory.controllerLevelLastTick != getMainController().level))
    return;
  const mainPath = getMainController().pos
    .findPathTo(
      Game.spawns["Spawn1"].room.find(FIND_SOURCES)[0]);
  if (!mainPath[mainPath.length * .5])
    return;
  getMainRoom().createFlag(
    mainPath[mainPath.length * .5].x,
    mainPath[mainPath.length * .5].y,
    "mainFlag"
  );
  const pathSpawnToFlag = Game.spawns["Spawn1"].pos.findPathTo(
    getMainRoom().find(FIND_FLAGS)[0]
  );
  for (const element of pathSpawnToFlag) {
    getMainRoom().createConstructionSite(
      element.x,
      element.y,
      STRUCTURE_ROAD
    );
  }
  const pathControllerToFlag = getMainController().pos.findPathTo(
    getMainRoom().find(FIND_FLAGS)[0]
  );
  ...
  const pathRessourceOneToFlag = getMainRoom().find(FIND_FLAGS)[0].pos.findPathTo(
    getMainRoom().find(FIND_SOURCES)[0]
  );
  for (let i = 0; i < pathRessourceOneToFlag.length; i++) {
    if (i == pathRessourceOneToFlag.length - 1)
      break;
    getMainRoom().createConstructionSite(
      pathRessourceOneToFlag[i].x,
      pathRessourceOneToFlag[i].y,
      STRUCTURE_ROAD
    );
  }

  const pathRessourceTwoToFlag = getMainRoom().find(FIND_FLAGS)[0].pos.findPathTo(
    getMainRoom().find(FIND_SOURCES)[1]
  );
  for (let i = 0; i < pathRessourceTwoToFlag.length; i++) {
    if (i == pathRessourceTwoToFlag.length - 1)
      break;
    getMainRoom().createConstructionSite(
      pathRessourceTwoToFlag[i].x,
      pathRessourceTwoToFlag[i].y,
      STRUCTURE_ROAD
    );
  }
...
export function buildBase(room: Room) {
  ...
  while (queue.length != 0) {
    ...
    if (currentPositionX < 0
      ...
      continue;
    const neighbourOne = new RoomPosition(
      currentPositionX + 1,
      currentPositionY,
      currentPosition.roomName
    );
    const neighbourTwo = new RoomPosition(
      currentPositionX - 1,
      currentPositionY,
      currentPosition.roomName
    );
    const neighbourThree = new RoomPosition(
      currentPositionX,
      currentPositionY + 1,
      currentPosition.roomName
    );
    const neighbourFour = new RoomPosition(
      currentPositionX,
      currentPositionY - 1,
      currentPosition.roomName
    );
    ...
    if (currentPosition.lookFor(LOOK_STRUCTURES).length > 0
      || currentPosition.lookFor(LOOK_SOURCES).length > 0)
    ...
    } else if (currentPositionX == spawnPosition.x + 1 && currentPosition.y == spawnPosition.y + 1
    ...
    } else if (currentPositionX == spawnPosition.x + 5 && currentPositionY == spawnPosition.y
    ...
    } else if (currentPositionX == spawnPosition.x + 5
    ...
    } else if ((neighbourOne.lookFor(LOOK_STRUCTURES).length == 0)
